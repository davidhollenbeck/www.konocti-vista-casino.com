...
Organizer.prototype.list = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var division = document.createElement("a");
        division.setAttribute('href', data[i].url);
        ...
        paragraph.setAttribute('class', 'calendar__title');
        paragraph.appendChild(document.createTextNode(data[i].text));
        division.appendChild(paragraph);

        var subtitle = document.createElement("P");
        subtitle.appendChild(document.createTextNode(data[i].subtitle));
        subtitle.setAttribute('class', 'calendar__subtitle');
        division.appendChild(subtitle);

        if ( data[i].eventType == 'event' ) {
            var eventText = document.createElement("P");
            eventText.appendChild(document.createTextNode("Buy Tickets"));
            eventText.setAttribute('class', 'calendar__eventText');
            division.appendChild(eventText);
        }
